...
public class Ex11_반복문연습예제1 {
	...
	public static void main(String[] args) {
		 ...
		 * 8과 12의 최대 공약수 : 4
		 * 
		 * 반복횟수 : i 1부터 num1까지 1씩 증가
		 * 실행문 : i가 num1의 약수이고 i가 num2의 약수이면 i를 출력 (공약수)
		 * => 조건식 : num1을 i로 나누었을 때 나머지가 0과 같고 num2를 i로 나누었을 때 나머지가 0과 같다면 / 실행문 : i를 출력
		 * 반복문 종료 후 : gcd 출력
		 */
		 ...
		 * 8의 배수 : 8, 16, 24, 32, 40, 48, 56, 64, 72...
		 ...
		 * => 조건식 : i를 num3으로 나누었을 때 나머지가 0과 같고, i를 num4로 나누었을 때 나머지가 0과 같다면 / 실행문 : i를 출력하고 종료
		 */
		...
		for(int i = 1; ; i++) {
			if(i % num3 == 0 && i % num4 == 0) {
				lcm = i;
				System.out.println(num3 + "과 " + num4 + "의 최소 공배수 : " + lcm);
				break;
			}
		}
		...
		//	=> 위의 예제와 결과는 같지만 반복횟수와 비교횟수가 더 적어 더 효율적인 방법
		 ...
		 * => 조건식 : num를 i로 나누었을 때 나머지가 0과 같다면 / 출력문 : i를 출력
		 */
		int num5 = 6;
		for(int i = 1; i <= num5; i++) {
			if(num5 % i == 0) {
				System.out.println(num5 + "의 약수 : " + i);
			}
		}
		 ...
		 * 반복문 종료 후 : count가 2면 소수라고 출력하고 아니면 소수가 아님이라고 출력
		 */
		int num = 6;
		int count = 0;
		for(int i = 1; i <= num; i++) {
