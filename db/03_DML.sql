...
# 1. 데이터 추가 : insert
# 1-1. 추가할 데이터를 다 아는 경우 : inset values
# - 문법
#	inset into 테이블명(컬럼1, 컬럼2, ..., 컬럼N)
#	values(값1, 값2, ..., 값N), (값1, 값2, ..., 값N); 
#	- 설명 : 생략된 컬럼들은 기본값이 지정되어 있어야 함
#			테이블의 컬럼 순서와 insert할 때 컬럼 순선느 달라도 됨
#- 문법2
#	insert into 테이블명
#	values(값1, 값2, ..., 값N)
#	- 설명 : 갑들은 테이블에 있는 컬럼 수만큼 써야 하고, 컬럼 순서대로 값 지정

#컴퓨터공학과, 학과 코드는 160, 주소는 1캠퍼스 A관 401호, 학과장은 미정
insert into major(code, address, name, boss_id)
values(160, 'A관 401호', '컴퓨터공학과', null);

# 유아교육과, 학과 코드는 456, 주소는 4캠퍼스 B관 101호, 학과장은 미정
insert into major
values('456', '유아교육과', null, 'B관 102호');

# 1-2. 검색 결과를 활용해서 데이터를 추가하는 경우 : inset select
...
# 워크벤치에서 unique가 아닌 속성을 조건에 쓰면 삭제가 안 됨
