...
# - 이영희, 전공 : 요가, 경력 2년
# - 홍길동, 전공 : 보디빌딩, 경력 3년 
insert into trainer(name, major, history) values('이영희', '요가', 2), ('홍길동', '보디빌딩', 3);
...
# 사물함을 1번부터 15번까지 등록
# 1~5번 : 직접추가
insert into locker values(null,null),(null,null),(null,null),(null,null),(null,null);
...
# 강좌3
# - 강좌명 : 크로스핏 초급반, 정원 : 30, 수강료 : 50,000 담당 : 김철수(1)

insert into program(title, personnel, fee, trainer_id) values('모닝 요가', 20, 100000, 2), ('지옥의 크로스핏', 15, 150000, 3), ('크로스핏 초급반', 30, 50000, 1);

# 수강 신청
# - 이순신(1) 회원이 모닝 요가(1)와 지옥의 크리스핏(2)을 수강 신청 후 결재 완료 함
insert into course(user_id, program_id) values(1, 1), (1, 2);

# - 성춘향(2) 회원이 모닝 요가(1)를 수강 신청 함
insert into course(user_id, program_id, state) values(2, 1, '신청');

# - 결재 완료하면 '결제 완료', 수강신청만 하면 '신청'으로 관리


# 출석체크 
# 이순신(1) 회원이 지옥의 크로스핏 강좌(2)를 출석체크 함 (현재시간)
insert attendance(date, state, user_id, program_id) values(curdate(), 'O', 1, 2); 
